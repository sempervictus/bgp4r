...
module BGP
  class Route_refresh < Message
    ...
    def afi=(val)
      raise ArgumentError, 'invalid argument' unless val.is_a?(Integer) and (0..0xffff).include?(val)

      @afi = val
    end

    def safi=(val)
      raise ArgumentError, 'invalid argument' unless val.is_a?(Integer) and (0..0xff).include?(val)
  ...
  class Orf
    def self.factory(s)
      ...
      when ORF::PREFIX
        ...
        raise "orf type #{type} not implemented"
      end
    end
  end
  ...
  class Orf_route_refresh < Message
    ...
    def parse(s)
      ...
      @orfs << Orf.factory(orfs.is_packed) while orfs.size > 0
    end
