    ...
    class Base_Capabilities
      ...
      def method_missing(name, *args, &block)
        if /^(send|recv)_(ipv4|ipv6|inet|inet6)_(.+)\?$/ =~ name.to_s
          ...
          when 'ipv4', 'inet' then afi = IANA.afi?(:ipv4)
          when 'ipv6', 'inet6' then afi = IANA.afi?(:ipv6)
          else
            super
          end
          case ::Regexp.last_match(3)
          when 'unicast' then safi = IANA.safi?(:unicast)
          when 'multicast' then safi = IANA.safi?(:multicast)
          when 'mpls_vpn_unicast' then safi = IANA.safi?(:mpls_vpn_unicast)
          when 'mpls_vpn_multicast' then safi = IANA.safi?(:mpls_vpn_multicast)
          else
            super
          end
          def_method(name, ::Regexp.last_match(1), afi, safi)
        else
          super
        end
      end
