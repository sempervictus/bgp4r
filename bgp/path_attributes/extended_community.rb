...
module BGP
  module ATTR::XTENDED_COMMUNITY
    ...
    def parse(s)
      ...
      when TWO_OCTET_AS then @global, @local = s.unpack('nN')
      when IPV4_ADDR
  ...
  class Extended_community
    ...
    def self.factory(_s)
      s = if _s.is_a?(Integer)
            [format('%16.16x', _s)].pack('H*')
          else
            _s
          end
      type, subtype, = s.unpack('CCa6')
      case subtype
      when ROUTE_TARGET then Route_target.new(s)
      when ROUTE_ORIGIN then Route_origin.new(s)
      when OSPF_DOMAIN_ID then Ospf_domain_id.new(s)
      when OSPF_ROUTER_ID then Ospf_router_id.new(s)
      when BGP_DATA_COLLECT then Bgp_data_collect.new(s)
      when LINK_BANDWIDTH then Link_bandwidth.new(s)
      when COLOR then Color.new(s)
      when BGP_ORIGIN_VALIDATION_STATE then Origin_validation_state.new(s)
