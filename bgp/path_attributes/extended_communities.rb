...
module BGP
  class Extended_communities < Attr
      ...
      def new_hash(*args)
        ...
        args.each do |arg|
          arg.keys.each do |comm|
            case comm
            when :extended_communities then o = new_hash(*args[0][:extended_communities])
            when :color then o << Color.new(*arg[comm])
            when :route_target then o << Route_target.new(*arg[comm])
            when :link_bandwidth then o << Link_bandwidth.new(*arg[comm])
            when :ospf_domain_id then o << Ospf_domain_id.new(*arg[comm])
            when :encapsulation then o << Encapsulation.new(*arg[comm])
            when :route_origin then o << Route_origin.new(*arg[comm])
            when :ospf_router_id then o << Ospf_router_id.new(*arg[comm])
            else
              raise
            end
          end
    ...
    def parse(s)
      ...
      self << Extended_community.factory(value.slice!(0, 8).is_packed) while value.size > 0
    end
