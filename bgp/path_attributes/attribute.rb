  ...
  module ATTR
    ...
    def _parse_(s, vf = nil)
      ...
      if flags & 0x10 > 0
        arr = s.unpack('CCn')
        len = arr[2]
        value = s[4..-1].unpack(vf || "a#{len}")
        s.slice!(0, arr[2] + 4).is_packed
      else
        arr = s.unpack('CCC')
        len = arr[2]
        value = s[3..-1].unpack(vf || "a#{len}")
    ...
    def to_s(value = '', mode = :tcpdump, as4byte = false)
      ...
      mode = :brief unless %i[tcpdump brief hexlify].include?(mode)

      case mode
      when :brief
        format '%s %4s %10s: [%s]%s', flags, "(#{@type})", attribute_name, shex,
               "#{value.size > 0 ? " '#{value}'" : ''}"
      when :hexlify
        s = format '%s %4s %10s: [%s]%s', flags, "(#{@type})", attribute_name, shex,
                   "#{value.size > 0 ? " '#{value}" : ''}"
        s += "\n\n"
        if as4byte
      ...
      when :tcpdump
        if as4byte
          _, _, len, enc_value = _parse_(encode4, nil)
        else
