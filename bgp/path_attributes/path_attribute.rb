  ...
  class Path_attribute
    ...
    def initialize(*args)
      if args.size <= 2 and args[0].is_a?(String) and args[0].is_packed?
        ...
        @attributes = []
        @attributes = args[0].keys.collect do |attr|
          case attr
          when :next_hop, :nexthop then Next_hop.new(args[0][attr])
          when :local_pref then Local_pref.new(args[0][attr])
          when :med, :multi_exit_disc then Multi_exit_disc.new(args[0][attr])
          when :mp_reach then Mp_reach.new(args[0][attr])
          when :mp_unreach then Mp_unreach.new(args[0][attr])
          when :as_path then As_path.new_hash(args[0][attr])
          when :as4_path then As4_path.new_hash(args[0][attr])
          when :origin then Origin.new(args[0][attr])
          when :originator_id then Originator_id.new(args[0][attr])
          when :cluster_list then Cluster_list.new(*args[0][attr])
          when :aggregator then Aggregator.new(args[0][attr])
          when :as4_aggregator then As4_aggregator.new(args[0][attr])
          when :communities, :community then Communities.new(args[0][attr])
          when :aigp then Aigp.new(args[0][attr])
          when :extended_communities, :extended_community
            Extended_communities.new_hash(*args[0][attr])
          when :atomic_aggregate then Atomic_aggregate.new
    ...
    def to_s(method = :default, as4byte = false)
      'Path Attributes:' + ([''] + @attributes.collect do |a|
    ...
    def [](type)
      return find_by_type(type) if type.is_a?(Integer)
    ...
    def att_sym_to_klass(sym)
      case sym
      when :communities, :community then Communities
      end
    end
    ...
    def method_missing(name, *args, &block)
      if name.to_s == 'med='
        ...
        replace(As_path, As_path.send("new_#{::Regexp.last_match(1)}", *args))
      else
        super
      end
    end
