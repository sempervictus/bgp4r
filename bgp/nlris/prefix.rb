...
module BGP
  class Prefix
    ...
    def self.new_ntop_extended(arg, afi = 1)
      raise "#{afi}" unless (1..3).include?(afi) || %i[nsap ipv4 ipv6].include?(afi)
    ...
    class << self
      def new_ntop(arg, afi = 1, path_id = nil)
        ...
        when IANA::AFI::NSAP, :nsap
          ...
          pfx = [o1, r.scan(/..../).collect { |_x| r.slice!(0, 4) }, r].flatten.join('.').chomp('.') + '/' + plen.to_s
        else
          raise
        end
