...
module BGP
  class Prefix_entry < Orf::Entry
    ...
    def match=(val)
      case val
      when 0, :permit then @match = 0
      when 1, :deny then @match = 1
      else
        raise
      end
    end
    ...
    def self.add(*args)
      Prefix_entry.new(0, *args)
    end

    def self.add_and_deny(*args)
      Prefix_entry.new(0, 1, *args)
    end

    def self.add_and_permit(*args)
      Prefix_entry.new(0, 0, *args)
    end

    def self.remove(*args)
      Prefix_entry.new(1, *args)
    end

    def self.remove_and_deny(*args)
      Prefix_entry.new(1, 1, *args)
    end

    def self.remove_and_permit(*args)
      Prefix_entry.new(1, 0, *args)
    end

    def self.remove_all(*_args)
      Prefix_entry.new(2, 0)
    end
  end
end
...
class BGP::Prefix_orf < BGP::Orf
  ...
  def _parse_(s)
    ...
    @entries << BGP::Prefix_entry.new(entries.is_packed) while entries.size > 0
  end
